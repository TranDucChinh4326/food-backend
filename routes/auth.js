...
async function sendVerificationEmail(email, fullname, verificationUrl) {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      to: email,
      subject: "Xác thực tài khoản Bếp 1979",
      html: `
        <p>Chao ${String(fullname || "ban")},</p>
        <p>Bam vao liên kết ben duoi de xác thực tài khoản Bếp 1979:</p>
...
async function sendPasswordResetEmail(email, fullname, otp) {
  ...
  if (gmailTransport) {
    await gmailTransport.sendMail({
      ...
      to: email,
      subject: "Ma OTP dat lai mat khau Bếp 1979",
      html: `
        <p>Chao ${String(fullname || "ban")},</p>
        <p>Ma OTP dat lai mat khau Bếp 1979 cua ban la:</p>
...
async function findProviderUser(provider, providerId) {
  ...
  // Social login dùng hàm này để quyết định đăng nhập ngay hay yêu cầu tạo tài khoản Bếp 1979.
...
async function createAuthProvider(userId, { provider, providerId, email }) {
  // Ghi quan hệ giữa tài khoản Bếp 1979 và provider đăng nhập.
...
router.post("/forgot-password", async (req, res) => {
  ...
  try {
    const normalizedEmail = normalizeEmail(req.body.email);
    const genericMessage = "Nếu email tồn tại, Bếp 1979 đã gửi hướng dẫn đặt lại mật khẩu.";
    ...
    res.json({
      message: "Bếp 1979 đã gửi mã OTP đặt lại mật khẩu đến email của bạn.",
